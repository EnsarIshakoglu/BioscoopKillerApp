"use strict"; var userAgent = navigator.userAgent.toLowerCase(), initialDate = new Date(), $document = $(document), $window = $(window), $html = $("html"), $body = $("body"), isDesktop = $html.hasClass("desktop"), isIE = userAgent.indexOf("msie") !== -1 ? parseInt(userAgent.split("msie")[1], 10) : userAgent.indexOf("trident") !== -1 ? 11 : userAgent.indexOf("edge") !== -1 ? 12 : false, isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent), windowReady = false, isNoviBuilder = false, isTouch = "ontouchstart" in window, plugins = { pointerEvents: isIE && isIE < 11 ? 'js/pointer-events.min.js' : false, tooltip: $('[data-toggle="tooltip"]'), search: $(".rd-search"), searchResults: $('.rd-search-results'), lightGallery: $("[data-lightgallery='group']"), lightGalleryItem: $("[data-lightgallery='item']"), lightDynamicGalleryItem: $("[data-lightgallery='dynamic']"), owl: $('.owl-carousel'), rdNavbar: $(".rd-navbar"), dateCountdown: $('.DateCountdown'), counter: $('.counter'), viewAnimate: $('.view-animate'), progressLinear: $(".progress-linear"), circleProgress: $(".progress-bar-circle"), swiper: $(".swiper-slider"), isotope: $(".isotope-wrap"), rdMailForm: $(".rd-mailform"), rdInputLabel: $(".form-label"), regula: $("[data-constraints]"), radio: $("input[type='radio']"), checkbox: $("input[type='checkbox']"), captcha: $('.recaptcha'), mailchimp: $('.mailchimp-mailform'), maps: $(".google-map-container"), selectFilter: $("select"), preloader: $(".preloader"), stepper: $("input[type='number']"), }, $year = $(".copyright-year"), i = 0; function isScrolledIntoView(elem) { if (isNoviBuilder) return true; return elem.offset().top + elem.outerHeight() >= $window.scrollTop() && elem.offset().top <= $window.scrollTop() + $window.height(); }
...
$window.on('load', function () {
    ...
    function initOwlCarousel(c) {
        ...
        responsive[values[j]] = {}; for (var k = j; k >= -1; k--) {
            ...
            if (!responsive[values[j]]["dotsEach"] && c.attr("data" + aliaces[k] + "dotsEach")) { responsive[values[j]]["dotsEach"] = k < 0 ? 1 : parseInt(c.attr("data" + aliaces[k] + "dotsEach"), 10); }
        }
        }
        ...
        c.on("initialized.owl.carousel", function () { initLightGalleryItem(c.find('[data-lightgallery="item"]'), 'lightGallery-in-carousel'); }); c.owlCarousel({ autoplay: isNoviBuilder ? false : c.attr("data-autoplay") === "true", loop: isNoviBuilder ? false : c.attr("data-loop") !== "false", items: 1, center: c.attr("data-center") === "true", dotsContainer: c.attr("data-pagination-class") || false, navContainer: c.attr("data-navigation-class") || false, mouseDrag: isNoviBuilder ? false : c.attr("data-mouse-drag") !== "false", nav: c.attr("data-nav") === "true", dots: (isNoviBuilder && c.attr("data-nav") !== "true") ? true : c.attr("data-dots") === "true", animateIn: c.attr('data-animation-in') ? c.attr('data-animation-in') : false, animateOut: c.attr('data-animation-out') ? c.attr('data-animation-out') : false, responsive: responsive, navText: c.attr("data-nav-text") ? $.parseJSON(c.attr("data-nav-text")) : [], navClass: c.attr("data-nav-class") ? $.parseJSON(c.attr("data-nav-class")) : ['owl-prev', 'owl-next'], onInitialized: function () { if (c.attr("data-active")) { c.trigger("to.owl.carousel", c.attr("data-active") - 1); } } });
    }
        ...
        var i, j, k, msg = { 'MF000': 'Successfully sent!', 'MF001': 'Recipients are not set!', 'MF002': 'Form will not work locally!', 'MF003': 'Please, define email field in your form!', 'MF004': 'Please, define type of your form!', 'MF254': 'Something went wrong with PHPMailer!', 'MF255': 'Aw, snap! Something went wrong.' }; for (i = 0; i < plugins.rdMailForm.length; i++) {
            var $form = $(plugins.rdMailForm[i]), formHasCaptcha = false; $form.attr('novalidate', 'novalidate').ajaxForm({
                data: { "form-type": $form.attr("data-form-type") || "contact", "counter": i }, beforeSubmit: function (arr, $form, options) {
                    ...
                    result = result.length === 5 ? result : 'MF255'; output.text(msg[result]); if (result === "MF000") { if (output.hasClass("snackbars")) { output.html('<p><span class="icon text-middle fa-check icon-xxs"></span><span>' + msg[result] + '</span></p>'); } else { output.addClass("active success"); } } else { if (output.hasClass("snackbars")) { output.html(' <p class="snackbars-left"><span class="icon icon-xxs fa-exclamation-triangle text-middle"></span><span>' + msg[result] + '</span></p>'); } else { output.addClass("active error"); } }
    ...
    if (plugins.selectFilter.length) {
        for (var i = 0; i < plugins.selectFilter.length; i++) {
            var
            select = $(plugins.selectFilter[i]), opts = { placeholder: select.attr("data-placeholder") ? select.attr("data-placeholder") : false, minimumResultsForSearch: select.attr("data-minimum-results-search") ? select.attr("data-minimum-results-search") : -1, maximumSelectionSize: 3, dropdownParent: $('.page'), containerCssClass: select.attr("data-container-class") ? select.attr("data-container-class") : '', dropdownCssClass: select.attr("data-dropdown-class") ? select.attr("data-dropdown-class") : '', }; select.select2(opts);
        }
    }
    ...
    if (isIE) {
        ...
        if (plugins.pointerEvents) { $.getScript(plugins.pointerEvents).done(function () { $html.addClass('lt-ie-11'); PointerEventsPolyfill.initialize({}); }); }
        if (isIE < 10) { $html.addClass('lt-ie-10'); }
    }
    if (plugins.tooltip.length) { for (i = 0; i < plugins.tooltip.length; i++) { var tooltipItem = plugins.tooltip[i]; $(tooltipItem).tooltip(); } }
    if ($.length) { $.RDToggles(); }
    if (plugins.dateCountdown.length) { for (i = 0; i < plugins.dateCountdown.length; i++) { var dateCountdownItem = plugins.dateCountdown[i], time = { "Days": { "text": "Days", "color": "#d81a24", "show": true }, "Hours": { "text": "Hours", "color": "#d81a24", "show": true }, "Minutes": { "text": "Minutes", "color": "#d81a24", "show": true }, "Seconds": { "text": "Seconds", "color": "#d81a24", "show": true } }; $(dateCountdownItem).TimeCircles({ "animation": "smooth", "bg_width": 1.0, "fg_width": 0.05666666666666667, "circle_bg_color": "rgba(255,255,255,1)", "time": time }); $(window).on('load resize orientationchange', function () { if (window.innerWidth < 479) { $(dateCountdownItem).TimeCircles({ time: { Minutes: { show: true }, Seconds: { show: false } } }).rebuild(); } else if (window.innerWidth < 767) { $(dateCountdownItem).TimeCircles({ time: { Seconds: { show: false } } }).rebuild(); } else { $(dateCountdownItem).TimeCircles({ time: time }).rebuild(); } }); } }
    ...
    if (plugins.rdNavbar.length) {
        ...
        plugins.rdNavbar.RDNavbar({
            autoHeight: false, anchorNav: !isNoviBuilder, stickUpClone: (plugins.rdNavbar.attr("data-stick-up-clone") && !isNoviBuilder) ? plugins.rdNavbar.attr("data-stick-up-clone") === 'true' : false, responsive: responsiveNavbar, callbacks: {
    ...
    $('.carousel').carousel()
}); $('.anchor').click(function () { var target = $(this).attr('href'); $('html, body').animate({ scrollTop: $(target).offset().top }, 800); return false; });; (function (window, $, undefined) {
    var conf = { center: true, backgroundControl: false }; var cache = { $carouselContainer: $('.thumbnails-carousel').parent(), $thumbnailsLi: $('.thumbnails-carousel li'), $controls: $('.thumbnails-carousel').parent().find('.carousel-control') }; function init() {
        cache.$carouselContainer.find('ol.carousel-indicators').addClass('indicators-fix'); cache.$thumbnailsLi.first().addClass('active-thumbnail'); if (!conf.backgroundControl) { cache.$carouselContainer.find('.carousel-control').addClass('controls-background-reset'); }
...
})(window, jQuery); $('.thumbnails-carousel').thumbnailsCarousel();